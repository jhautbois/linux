...
int skl_int3472_tps68470_probe(struct i2c_client *client)
{
	...
	/* First, setup the regmap that will be used by each pdev's driver */
	 ...
	 * scenarios:
	 *
	 * 	1. No CLDB - likely ACPI tables designed for Chromium. We create
	...
	if (cldb_present) {
		clk_dev = skl_int3472_register_pdev("tps68470-clk", &client->dev);
		if (IS_ERR(clk_dev)) {
			ret = PTR_ERR(clk_dev);
			goto err_free_gpio;
		}

		regulator_dev = skl_int3472_register_pdev("tps68470-regulator", &client->dev);
		if (IS_ERR(regulator_dev)) {
			ret = PTR_ERR(regulator_dev);
			goto err_free_clk;
		}
	} else {
		opregion_dev = skl_int3472_register_pdev("tps68470_pmic_opregion", &client->dev);
